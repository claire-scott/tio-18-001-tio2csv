...
def get_config():
    ...
    parser.add('-c', '--my-config', is_config_file=True,
               help='config file path (will load tio2csv.config by default)')
    ...
    parser.add_argument(
        '--tio_access_key',
        help='Tenable.io API access key (can also be provided by environment variable TIO_ACCESS_KEY)',
        required=True,
        env_var='TIO_ACCESS_KEY')
    parser.add_argument(
        '--tio_secret_key',
        help='Tenable.io API secret key (can also be provided by environment variable TIO_SECRET_KEY)',
        required=True,
        env_var='TIO_SECRET_KEY')

    parser.add_argument(
        '--temp_file_dir',
        help='Temporary directory for vulnerability export files from Tenable.io (defaults to system temporary directory)',
        default=tempfile.gettempdir())
    parser.add_argument(
        '--temp_file_name',
        help='filename for vulnerability export file (defaults to vulns_1.json...)',
        default='vulns_%(chunk_id)s.json')
    parser.add_argument(
        '--keep_temp_files',
        type=bool,
        help='Keep vulnerability download files (defaults to false)',
        default=False)

    parser.add_argument(
        '--vuln_state',
        action='append',
        help='The list of vulnerability states to include in the export, options are [OPEN, REOPENED, FIXED] (defaults to [open,reopened])',
        default=[
            'OPEN',
            'REOPENED'])
    parser.add_argument(
        '--vuln_severity',
        action='append',
        help='The list of vulnerability severity levels to include in the export, options are [[info, low, medium, high, critical] (defaults to [high,critical])',
        default=[
            'high',
            'critical'])
    ...
    parser.add_argument(
        '--csv_header_row',
        type=bool,
        help='Should the csv file contain a header row with the column names (defaults to True)',
        default=True)
    parser.add_argument(
        '--csv_columns',
        action='append',
        help='The list of columns to export to the CSV file, see config file for available columns and default')
    parser.add_argument(
        '--csv_column_names',
        action='append',
        help='List of column header names to use in output CSV file, if not provided and csv_header_row is set to true, it will use the default column names')
    parser.add_argument(
        '--csv_null_value',
        default='NULL',
    ...
    # the csv file
    parser.add_argument(
        '--csv_newline_character',
        default='\n',
        help='The character used to indicate a new line in the csv file')
    parser.add_argument(
        '--csv_delimiter',
        default=',',
        help='The character used to indicate a new field in the csv file')
    parser.add_argument(
        '--csv_quote_char',
        default='"',
        help='The character used to indicate a new field in the csv file')
    parser.add_argument(
        '--csv_quote_everything',
        type=bool,
        default=False,
        help='Set to true to surround every element in the CSV file with quotes')
...
def get_redacted_parser_values(parser):
    ...
    for i in range(len(lines)):
        if re.match(r'^\s*(tio_access_key|tio_secret_key):.*$',
...
def main():
    ...
    client = TenableIOClient(
        access_key=options.tio_access_key,
    ...
    try:
        ...
        chunks_available = client.export_helper.download_vulns(
            path=file_name, state=options.vuln_state, severity=options.vuln_severity)
    except Exception:
